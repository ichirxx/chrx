...
def get_accounts_file():
    ...
    if is_termux():
        ...
        if os.path.isdir(shared_storage):
            save_dir = os.path.join(shared_storage, 'M_ACCOUNTS')
            try:
        ...
        if os.path.isdir(downloads):
            save_dir = os.path.join(downloads, 'M_ACCOUNTS')
            try:
                os.makedirs(save_dir, exist_ok=True)
                return os.path.join(save_dir, 'EPBI.txt')
            except Exception:
        ...
        home_dir = os.path.expanduser('~/M_ACCOUNTS')
...
def show_accounts_location():
    ...
    if is_termux():
        ...
        if os.path.isdir(shared):
            print(f'{Colors.GREEN}💾 Accounts saved to: Internal Storage → M_ACCOUNTS/accounts.txt{Colors.RESET}')
            print(f'{Colors.CYAN}   (Open your file manager and go to Internal Storage/M_ACCOUNTS){Colors.RESET}')
        else:
...
def show_post_creation_tips():
    ...
    print(f'{Colors.CYAN}   • lcxmail.site email domain{Colors.RESET}')
    ...
    print(f'{Colors.YELLOW}   Step 3: Login with email@lcxmail.site + password{Colors.RESET}')
    ...
    print(f'   3️⃣  Enter: username@lcxmail.site')
    ...
    print(f'   Spread lcxmail.site confirmations across TIME, not just DEVICES!')
...
while True:
    ...
    while True:
        ...
        while True:
            ...
            print(f'  {Colors.YELLOW}[1]{Colors.RESET}  {Colors.WHITE}lcxmail.site {Colors.GREEN}★ DEFAULT{Colors.RESET}')
            ...
            print(f'  {Colors.YELLOW}[5]{Colors.RESET}  {Colors.WHITE}ygmail.cfd{Colors.RESET}')
            print(f'     {Colors.WHITE}Back{Colors.RESET}')
            print(f'{Colors.RED}{"─" * 44}{Colors.RESET}')
            ...
            while True:
                ...
                print(f'  {Colors.YELLOW}[2]{Colors.RESET}  {Colors.WHITE}Custom Password {Colors.RED}(default: lcxnorms){Colors.RESET}')
                print(f'     {Colors.WHITE}Back{Colors.RESET}')
                print(f'{Colors.RED}{"─" * 44}{Colors.RESET}')
                ...
                if password_choice not in ['1','2']:
                    password_choice = '2'
                    custom_password = 'lcxnorms'
                elif password_choice == '2':
                    custom_password = input(f'{Colors.RED}[►]{Colors.RESET} {Colors.WHITE}Password (Enter = lcxnorms){Colors.RESET}: ').strip()
                ...
                while True:
                    ...
                    try:
                        ...
                        if email_choice == '3' and num_accounts > 8:
                            ...
                            num_accounts = 8
                        # lcxmail.site unlimited mode notice
    ...
    for i in range(num_accounts):
        ...
        for attempt in range(5):
            try:
                ...
                if use_custom_domain and custom_domain == 'lcxmail.site':
                    payload['email_confirm_method'] = "manual"
                    payload['fb_lite_manual_email_mode'] = "true"
                    payload['lcxmail_site_domain_email'] = "true"
                    ...
                    payload['lcxmail_site_email_confirmation_type'] = "manual_fb_lite"
                    payload['skip_checkpoint_lcxmail_site'] = "true"
                    payload['unlimited_email_confirmations'] = "true"
                    payload['no_checkpoint_after_weyn_store_email'] = "true"
                    payload['lcxmail_site_fb_lite_compatible'] = "true"
                    payload['email_provider_custom_lcx'] = "true"
                ...
                # For lcxmail.site + original FB Lite: force NO cloned-app context.
