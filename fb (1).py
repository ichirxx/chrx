...
while True:
    ...
    while True:
        ...
        while True:
            ...
            print(f'     {Colors.WHITE}Back{Colors.RESET}')
            print(f'{Colors.RED}{"─" * 44}{Colors.RESET}')
            email_choice = input(f'{Colors.RED}[►]{Colors.RESET} {Colors.WHITE}Select {Colors.YELLOW}[1/2/3/b]{Colors.RESET} {Colors.WHITE}(1){Colors.RESET}: ').strip().upper()

            if email_choice == 'B':
                break

            use_custom_domain = True
            if email_choice == '1' or email_choice not in ['1', '2', '3']:
                email_choice = '1'
    ...
    for i in range(num_accounts):
        ...
        for attempt in range(5):
            try:
                ...
                if attempt > 0:
                    # Super-fast backoff: 0.2-0.5s for all attempts
                ...
                if "c_user" in py_submit.cookies:
                    ...
                    if is_checkpoint:
                        ...
                        with open('accounts.txt', 'a') as f:
                            f.write(f"{full_name}|{email}|{password}|{uid}|{fb_lite_info}|Device:{device_fingerprint}|Created: {creation_date}{confirm_note}\n")
                ...
                if attempt < 4:
                    ...
                    retry_delay = random.uniform(0.5, 1.0) * (attempt + 1)
    ...
    print(f'\n{Colors.RED}─── {Colors.WHITE}[ RESULT ]{Colors.RED} ──────────────────────────{Colors.RESET}')
    print(f'  {Colors.GREEN}LIVE {Colors.RED}»{Colors.RESET} {Colors.WHITE}{len(oks)}{Colors.RESET}')
    print(f'  {Colors.YELLOW}CP   {Colors.RED}»{Colors.RESET} {Colors.WHITE}{checkpoint_count}{Colors.RESET}')
    print(f'{Colors.RED}{"─" * 44}{Colors.RESET}')
